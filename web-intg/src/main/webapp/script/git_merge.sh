...
WK_BRANCH=$2
BRANCH_NAME=${3//,/ }
echo "work_base_dir:  $WORK_BASE_DIR"
cd $WORK_BASE_DIR
...
for BRANCH in $BRANCH_NAME
 do
  ...
  git merge --no-ff origin/$BRANCH  > /tmp/$MERGE_OUTPUT_FILE

  CONFLICT_FILES=`cat /tmp/$MERGE_OUTPUT_FILE | grep "CONFLICT (content): Merge conflict in" | awk -F"Merge conflict in " '{print $2}'`
  TREE_CFCL_FILES=`cat /tmp/$MERGE_OUTPUT_FILE | grep "CONFLICT (rename/modify): Merge conflict in" | awk -F"Merge conflict in " '{print $2}'`
  TREE_CFCL_FILES_RENAME=`cat /tmp/$MERGE_OUTPUT_FILE | grep "CONFLICT (rename/rename): Rename " | awk -F" Rename " '{print $2}' | awk -F"\"" '{print $4 }{print $10}'`
  echo "renameParam: $TREE_CFCL_FILES_RENAME" 
#  rm -f /tmp/$MERGE_OUTPUT_FILE
  
  if [ ! -z "$CONFLICT_FILES" -o ! -z "$TREE_CFCL_FILES" -o ! -z "$TREE_CFCL_FILES_RENAME" ];then
    echo "$3" | awk -F"$BRANCH" '{print $2}' > .git/LEFT_BRANCH    
    echo "$BRANCH" > .git/CFLC_BUSI_BRANCH
    ...
    if [ ! -z "$CONFLICT_FILES" ];then
      ...
      for F in $CONFLICT_FILES
		...
		echo "file here :$WORK_BASE_DIR/$WK_DIR/$F"
    ...
    if [ ! -z "$TREE_CFCL_FILES_RENAME" ];then
      echo $TREE_CFCL_FILES_RENAME > $WORK_BASE_DIR/$WK_DIR/.git/TREE_CFLC_FILE
      ...
      echo "#you can discuss with the branches '$ALREADY_MERGED_BRANCH' developers to resolve the conflict." >> tree_conflict.txt
      echo "" >> tree_conflict.txt
      echo $TREE_CFCL_FILES_RENAME >> tree_conflict.txt
